...
class Minify
{
	...
	private function _scan_files($type, $force, $group)
	{
		...
		if (is_array($files_array))
		{
			...
			{
				$filename = $this->_generate_file_name($file, $directory, $type, $key);
	...
	private function _generate_file_name($file, $directory, $type, $key)
	{
		if(is_numeric($key)){
			$filename = $directory . '/' . $file;
		}else{
			if($type == 'css'){
				$module_directory = $this->assets_dir . '/modules/' . $key . '/css';
			}else if ($type == 'js'){
				$module_directory = $this->assets_dir . '/modules/' . $key . '/js';
			}
			$filename = $module_directory . '/' . $file;
		}
	...
	private function _concat_files($file_array, $directory, $out_file, $type)
	{
		...
		if ($fh = fopen($out_file, 'w'))
		{
			...
			{
				$file_name = $this->_generate_file_name($file_name, $directory, $type, $key);
